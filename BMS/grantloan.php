...
include 'includes/dbconnect.php';
	...
	if(isset($_POST['submit'])){
			...
			if($temp>0){
				...
				$in_sql = "SELECT * FROM loan WHERE accno = '$accno'";
				$tr_qr = mysqli_query($con, $in_sql);

				if(!mysqli_num_rows($tr_qr)){
				...
				if($lamount>0){
					...
					$cnt = "SELECT * FROM loan";
					$cnt1 = mysqli_query($con, $cnt);
					$cnt2 = mysqli_num_rows($cnt1);
					$cnt2++;

					$ins_sql = "INSERT INTO loan (loanid,  loanamt, accno, interest, startdate,recentdate) VALUES ('".$cnt2."', '".$lamount."', '".$accno."', '".$linterest."', '".$ldate."','".$ldate."')";
					$run_sql = mysqli_query($con,$ins_sql);

					$cnt = "SELECT MAX(loanid) as maximum FROM loan ";
					$cnt_sql = mysqli_query($con,$cnt);
					$cnt = mysqli_fetch_assoc($cnt_sql);
					$maximum=$cnt['maximum'];

					$one=1;
					$sql5 = "INSERT INTO transactions(transactionid, paymentdate, payeeid,receiveid,amount) VALUES('".$maximum."', '".$ldate."','".$one."', '".$accno."','".$lamount."')";
					$run5 = mysqli_query($con, $sql5);
						$success = "Loan amount Granted successfully!";
				}else{

					$success = "Loan amount cannot be negative!";
				}
			}else{
				$success = "Already loan granted!";
			}
		}
		else{
    ...
    <div class="page-header">
			...
			<form class="form-horizontal" action="grantloan.php" method="post" role="form">
				<div class="form-group">
					<label for="number" class="col-sm-3 control-label">A/c no</label>
						<div class="col-sm-8">
